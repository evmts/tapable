...
class Hook {
  ...
  private _tap(type: string, options: string | TapOptions, fn: Function) {
    let _options: TapOptions =
      typeof options === 'string' ? { name: options.trim() } : options
  ...
  withOptions(options: TapOptions) {
    ...
    return {
      name: this.name,
      tap: (opt: string | TapOptions, fn: Function) =>
        this.tap(mergeOptions(opt), fn),
      tapAsync: (opt: string | TapOptions, fn: Function) =>
        this.tapAsync(mergeOptions(opt), fn),
      tapPromise: (opt: string | TapOptions, fn: Function) =>
        this.tapPromise(mergeOptions(opt), fn),
      intercept: (interceptor: Interceptor) => this.intercept(interceptor),
      isUsed: () => this.isUsed(),
      withOptions: (opt: string | TapOptions) =>
